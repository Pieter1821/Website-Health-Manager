...
def test_load_cloud_config_from_env(monkeypatch, tmp_path: Path) -> None:
    ...
    cfg = load_cloud_config(allow_bootstrap_token=True)
...
def test_desktop_rejects_bootstrap_token_from_env(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("WHM_API_URL", "https://example.workers.dev/")
    monkeypatch.setenv("WHM_API_TOKEN", "secret-token")
    monkeypatch.setattr(
        "whm.infrastructure.cloud_config.cloud_config_path",
...
def test_desktop_rejects_bootstrap_token_in_file(monkeypatch, tmp_path: Path) -> None:
    path = tmp_path / "cloud.json"
    path.write_text(
...
def test_session_preferred_over_bootstrap_token(monkeypatch, tmp_path: Path) -> None:
    path = tmp_path / "cloud.json"
    path.write_text(
        json.dumps(
            {
    ...
    cfg = load_cloud_config(allow_bootstrap_token=True)
    assert cfg is not None
    assert cfg.api_token == "aaa.bbb.ccc"


def test_expired_session_ignored_without_bootstrap(
...
def test_api_url_only_enables_cloud_without_credentials(
    monkeypatch, tmp_path: Path
) -> None:
    path = tmp_path / "cloud.json"
    path.write_text(
        json.dumps({"api_url": "https://api.example"}),
        encoding="utf-8",
    )
    monkeypatch.delenv("WHM_API_URL", raising=False)
    monkeypatch.delenv("WHM_API_TOKEN", raising=False)
