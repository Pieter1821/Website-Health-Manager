...
def build_services(db_path: Path | None = None):
    ...
    If ~/.whm/cloud.json (or WHM_API_URL) is set, use Cloudflare D1
    ...
    # Local DB is always available for secret settings (and full local mode).
    conn = connect(db_path or default_db_path())
    initialize_database(conn)
    local_settings = SqliteSettingsRepository(conn)

    if cloud and cloud.enabled and not force_local:
        logging.getLogger(__name__).info(
            "Using Cloudflare D1 via %s (authenticated=%s)",
            cloud.api_url,
            cloud.has_credentials,
        )
        ...
        settings = HybridSettingsRepository(
            CloudSettingsRepository(client),
            local_settings,
        )
        ...
        return website_service, scan_service, settings_service, conn, client
    ...
    website_service = WebsiteService(customers, websites)
    scan_service = HealthScanService(websites, health, dns, local_settings)
    settings_service = SettingsService(local_settings)
    return website_service, scan_service, settings_service, conn, None
